...
/**
 * 收費單 收費項目不在報表格式中的情況下,合計金額仍會加總,但報表中無法顯示該收費項目金額
 * 
 * @author shawn
 *
 */
public class TuitionManagerAction extends BasePrintXmlAction {
	...
	public String add() {
		...
		try {
			String ClassNo = df.sqlGetStr("SELECT ClassNo FROM Class WHERE CampusNo='" + cno + "'AND SchoolNo='" + sno
					+ "'AND DeptNo='" + dno + "'AND Grade='" + gno + "'AND SeqNo='" + zno + "'");
			df.exSql("DELETE FROM FeePay WHERE DepartClass='" + ClassNo + "'AND Fcode='N'AND Kind='2'");
	...
	private List getCs(boolean upgrade) {
		...
		StringBuilder sb = new StringBuilder(
				"SELECT d.dname,d.fname, fq.quota,fq.no, c.*,cc.name as CampusName,cd.name as DeptName,"
		...
		if (upgrade) {
			...
			if (term.equals("1")) {// 第1學期需升級
				for (int i = 0; i < list.size(); i++) {
					// 重整升級後人數
					list.get(i).put("cnt",
							df.sqlGetInt("SELECT COUNT(*) FROM stmd s, Class c WHERE "
									+ "s.depart_class=c.ClassNo AND c.CampusNo='" + list.get(i).get("CampusNo")
									+ "'AND SchoolNo='" + list.get(i).get("SchoolNo") + "'AND " + "DeptNo='"
									+ list.get(i).get("DeptNo") + "'AND Grade='"
									+ (Integer.parseInt(list.get(i).get("Grade").toString()) - 1) + "'AND SeqNo='"
									+ list.get(i).get("SeqNo") + "'"));
				}
			}
		}
		...
		for (int i = 0; i < list.size(); i++) {
			...
			list.get(i).put("pay1", df.sqlGet(
					"SELECT fc.Name, fc.No, fc.No as Fcode, fc.kind,(SELECT Oid FROM FeePay WHERE DepartClass=c.ClassNo AND Fcode=fc.No)as Oid,"
							+ "(SELECT Money FROM FeePay WHERE DepartClass=c.ClassNo AND Fcode=fc.No)as Money "
							+ "FROM Class c, (SELECT * FROM FeeCode)fc WHERE fc.kind='1'AND c.ClassNo='"
							+ list.get(i).get("ClassNo") + "'"));

			list.get(i).put("pay2", df.sqlGet(
					"SELECT fc.Name, fc.No, fc.No as Fcode, fc.kind,(SELECT Oid FROM FeePay WHERE DepartClass=c.ClassNo AND Fcode=fc.No)as Oid,"
							+ "(SELECT Money FROM FeePay WHERE DepartClass=c.ClassNo AND Fcode=fc.No)as Money "
							+ "FROM Class c, (SELECT * FROM FeeCode)fc WHERE fc.kind='2'AND c.ClassNo='"
							+ list.get(i).get("ClassNo") + "'"));
			// 學生數大於0時
	...
	private StringBuilder genSql(StringBuilder sb) {
		if (!cno.equals(""))
			sb.append("AND c.CampusNo='" + cno + "'");
		if (!sno.equals(""))
			sb.append("AND c.SchoolNo='" + sno + "'");
		if (!tno.equals(""))
			sb.append("AND c.SchoolType='" + tno + "'");
		if (!dno.equals(""))
			sb.append("AND c.DeptNo='" + dno + "'");

		if (!zno.equals(""))
			sb.append("AND c.SeqNo='" + zno + "'");

		if (scope.equals("")) {// 不設限新生
			if (!gno.equals(""))
				sb.append("AND c.Grade='" + gno + "'");
		} else {
			if (scope.equals("1"))
				sb.append("AND c.Grade='1'");// 只限新生
			if (scope.equals("2"))
				sb.append("AND c.Grade!='1'");// 只限舊生
		}
		...
		for (int i = 0; i < list.size(); i++) {
			...
			if (term.equals("1") && geNew) {
				stds = df.sqlGet("SELECT s.sex, s.student_no, s.student_name, s.idno FROM stmd s, Class c WHERE "
						+ "s.depart_class=c.ClassNo AND c.CampusNo='" + list.get(i).get("CampusNo") + "'AND SchoolNo='"
						+ list.get(i).get("SchoolNo") + "'AND " + "DeptNo='" + list.get(i).get("DeptNo")
						+ "'AND Grade='" + (Integer.parseInt(list.get(i).get("Grade").toString()) - 1) + "'AND SeqNo='"
						+ list.get(i).get("SeqNo") + "'ORDER BY s.student_no");
				if (geNew)
					if (stds.size() < 1 && list.get(i).get("quota") != null) {// 建立無學號
						stds = genStds(Integer.parseInt(list.get(i).get("quota").toString()),
								list.get(i).get("no").toString());
					}
			} else {
	...
	public String tuitionPrint() throws IOException, ParseException {
		...
		out.println("  <Table ss:ExpandedColumnCount='22' ss:ExpandedRowCount='" + (row + 1) + "' x:FullColumns='1'");
		...
		out.println("    <Cell ss:StyleID='s64'><Data ss:Type='String'>行政院減免學雜費</Data></Cell>");
		out.println("    <Cell ss:StyleID='s64'><Data ss:Type='String'>五專免學費</Data></Cell>");
		...
		for (int i = 0; i < list.size(); i++) {
			...
			for (int j = 0; j < stds.size(); j++) {
				ClassName = list.get(i).get("CampusName").toString().substring(0, 2)
						+ list.get(i).get("ClassName").toString();
				...
				if (stds.get(j).get("idno") == null) {
					out.println(
							"    <Cell><Data ss:Type='String'>" + stds.get(j).get("student_no") + "A</Data></Cell>");
				} else {
				...
				out.println("    <Cell><Data ss:Type='String'>" + list.get(i).get("CampusNo")
						+ list.get(i).get("SchoolNo") + list.get(i).get("DeptNo") + "</Data></Cell>");
				out.println("    <Cell><Data ss:Type='String'>" + list.get(i).get("fname") + list.get(i).get("dname")
						+ "</Data></Cell>");
				...
				if (pay1.get("D") == null) {
					...
					out.println("    <Cell><Data ss:Type='Number'>" + pay1.get("D") + "</Data></Cell>");
				}
				if (pay1.get("E") == null) {
					out.println("    <Cell><Data ss:Type='Number'>0</Data></Cell>");
				} else {
					out.println("    <Cell><Data ss:Type='Number'>" + pay1.get("E") + "</Data></Cell>");
				}
				out.println("    <Cell><Data ss:Type='Number'>" + list.get(i).get("mon1") + "</Data></Cell>");

				out.println("    <Cell><Data ss:Type='String'>" + Integer.parseInt(list.get(i).get("Grade").toString())
						+ "</Data></Cell>");
				// out.println (" <Cell ss:Index='20'><Data
				// ss:Type='String'>"+bl.getWeekOfDay4Zh(Integer.parseInt(list.get(i).get("Grade").toString()),
				// null)+"</Data></Cell>");
	...
	public String nottuitionPrint() throws IOException, ParseException {
		...
		out.println(" </Styles>");
		out.println(" <Worksheet ss:Name='SHEET1'>");
		out.println("  <Table ss:ExpandedColumnCount='99' ss:ExpandedRowCount='" + row + 99 + "' x:FullColumns='1'");
		out.println("   x:FullRows='1' ss:DefaultColumnWidth='54' ss:DefaultRowHeight='16.5'>");
		...
		// out.println (" <Cell ss:StyleID='s64'><Data
		// ss:Type='String'>存繳帳號</Data></Cell>");
		// out.println (" <Cell ss:StyleID='s64'><Data
		// ss:Type='String'>家長姓名</Data></Cell>");
		out.println("    <Cell ss:StyleID='s64'><Data ss:Type='String'>年級</Data></Cell>");
		out.println("    <Cell ss:StyleID='s64'><Data ss:Type='String'>繳款期限</Data></Cell>");
		out.println("   </Row>");
		String ClassName;
		for (int i = 0; i < list.size(); i++) {
			...
			stds = (List<Map>) list.get(i).get("stds");
			for (int j = 0; j < stds.size(); j++) {
				if (list.get(i).get("mon2").toString().equals("0"))
					continue;
				out.println("   <Row ss:AutoFitHeight='0'>");
				out.println("    <Cell><Data ss:Type='String'>" + stds.get(j).get("student_no") + "</Data></Cell>");
				out.println("    <Cell><Data ss:Type='String'>" + stds.get(j).get("student_name") + "</Data></Cell>");
				...
				// out.println (" <Cell><Data
				// ss:Type='String'>"+stds.get(j).get("idno")+"</Data></Cell>");
				out.println("    <Cell></Cell>");
				out.println("    <Cell></Cell>");
				out.println("    <Cell><Data ss:Type='String'>" + list.get(i).get("ClassNo") + "</Data></Cell>");
				out.println("    <Cell><Data ss:Type='String'>" + ClassName + "</Data></Cell>");
				...
				// out.println (" <Cell ss:StyleID='s64'><Data
				// ss:Type='Number'>"+((Map)list.get(i).get("pay2")).get("1")+"</Data></Cell>");
				out.println("    <Cell><Data ss:Type='Number'>0</Data></Cell>");

				// if(((Map)list.get(i).get("pay2")).get("8")==null)((Map)list.get(i).get("pay2")).put("8",
				// "");
				// out.println (" <Cell><Data
				// ss:Type='Number'>"+((Map)list.get(i).get("pay2")).get("8")+"</Data></Cell>");
				out.println("    <Cell><Data ss:Type='Number'>0</Data></Cell>");

				// if(((Map)list.get(i).get("pay2")).get("T")==null)((Map)list.get(i).get("pay2")).put("T","");
				// out.println (" <Cell><Data
				// ss:Type='Number'>"+((Map)list.get(i).get("pay2")).get("T")+"</Data></Cell>");
				out.println("    <Cell><Data ss:Type='Number'>0</Data></Cell>");

				if (((Map) list.get(i).get("pay2")).get("N") == null)
					((Map) list.get(i).get("pay2")).put("N", "");
				out.println("    <Cell><Data ss:Type='Number'>" + ((Map) list.get(i).get("pay2")).get("N")
						+ "</Data></Cell>");

				// if(((Map)list.get(i).get("pay2")).get("A")==null)((Map)list.get(i).get("pay2")).put("A",
				// "");
				// out.println (" <Cell><Data
				// ss:Type='Number'>"+((Map)list.get(i).get("pay2")).get("A")+"</Data></Cell>");
				out.println("    <Cell><Data ss:Type='Number'>0</Data></Cell>");

				if (list.get(i).get("mon2") == null)
					list.get(i).put("mon2", "");
				out.println("    <Cell><Data ss:Type='Number'>" + list.get(i).get("mon2") + "</Data></Cell>");

				out.println(
						"    <Cell><Data ss:Type='String'>" + list.get(i).get("Grade").toString() + "</Data></Cell>");
				out.println("    <Cell ss:StyleID='s64'><Data ss:Type='String'>" + edate + "</Data></Cell>");
				out.println("   </Row>");
			}
		}
	...
	public String acc_print() throws IOException, ParseException {
		...
		out.println("    <Cell ss:StyleID='s107'><Data ss:Type='String'>行政院補貼校內住宿費</Data></Cell>");
		...
		// public String acc_fee_m, acc_fee_f, sec_fee_m, sec_fee_f, air_fee_m,
		// air_fee_f;
		int sum_m, sum_f;
		...
		System.out.println(term.equals("1"));
		// 升級
		if (term.equals("1")) {
			String nc;
			Map oc;
			for (int i = 0; i < list.size(); i++) {
				// TODO 無班級的處理
				try {
					nc = df.sqlGetStr("SELECT ClassNo FROM Class WHERE CampusNo='" + list.get(i).get("CampusNo") + "' "
							+ "AND SchoolNo='" + list.get(i).get("SchoolNo") + "' AND DeptNo='"
							+ list.get(i).get("DeptNo") + "' AND " + "Grade='"
							+ (Integer.parseInt(list.get(i).get("Grade").toString()) + 1) + "' AND SeqNo='"
							+ list.get(i).get("SeqNo") + "'");
					...
					oc = df.sqlGetMap("SELECT*FROM Class WHERE ClassNo='" + nc + "'");
					oc.remove("stds");
					list.get(i).putAll(oc);
		...
		Map pay1;
		for (int i = 0; i < list.size(); i++) {
			System.out.println(list.get(i).get("ClassName") + " -er");
			try {
				stds = (List<Map>) list.get(i).get("stds");
			} catch (Exception e) {
				continue;
			...
			for (int j = 0; j < stds.size(); j++) {
				...
				if (stds.get(j).get("sex") == null)
					stds.get(j).put("sex", "1");
				if (stds.get(j).get("sex").equals("1")) {
					...
					out.println("    <Cell><Data ss:Type='Number'>-5000</Data></Cell>");
					out.println("    <Cell><Data ss:Type='Number'>0</Data></Cell>");
					out.println("    <Cell><Data ss:Type='Number'>" + (sum_m - 5000) + "</Data></Cell>");
				} else {
					...
					out.println("    <Cell><Data ss:Type='Number'>-5000</Data></Cell>");
					out.println("    <Cell><Data ss:Type='Number'>0</Data></Cell>");
					out.println("    <Cell><Data ss:Type='Number'>" + (sum_f - 5000) + "</Data></Cell>");
				}
		...
		for (int i = 0; i < ClassNo.length; i++) {
			if (!ClassNo[i].equals(""))
				if (quota[i].trim().equals("") || no[i].trim().equals("")) {
					...
					// df.exSql("INSERT INTO FeeQuota(ClassNo,quota,no) VALUES
					// ('"+ClassNo[i]+"','"+quota[i]+"','"+no[i]+"')ON DUPLICATE KEY UPDATE
					// quota='"+quota[i]+"',no='"+no[i]+"'");
				} else {
					df.exSql("INSERT INTO FeeQuota(ClassNo,quota,no) VALUES ('" + ClassNo[i] + "','" + quota[i] + "','"
							+ no[i] + "')ON DUPLICATE KEY UPDATE quota='" + quota[i] + "',no='" + no[i] + "'");
				}
		}
		return search();
	}

	public String minusAmount;// 減免金額
	...
	public String reduce() {
		...
		// df.exSql("DROP TABLE IF EXISTS FeePay_AUTOBACKUP;");
		// df.exSql("CREATE TABLE FeePay_AUTOBACKUP AS SELECT * FROM FeePay;");// 備份

		List<Map> s = getCs(true);
		delD(s);
		for (int i = 0; i < s.size(); i++) {
			df.exSql("INSERT INTO FeePay(DepartClass,Fcode,Kind,Money)VALUES('" + s.get(i).get("ClassNo")
					+ "','D','1', '-" + minusAmount + "');");
		}
		return search();
	}

	private void delD(List<Map> s) {

		StringBuilder sql = new StringBuilder("DELETE FROM FeePay WHERE Fcode='D' AND DepartClass IN(");
		for (int i = 0; i < s.size(); i++) {
			sql.append("'" + s.get(i).get("ClassNo") + "',");
		}
	 ...
	 * @return
	 */
	public String reduceHalf() {

		List<Map> s = getCs(true);

		delD(s);
		float minu1, minu2, sum;// 學分費, 雜費
		for (int i = 0; i < s.size(); i++) {
			try {
				minu1 = Integer.parseInt(df.sqlGetStr("SELECT Money FROM FeePay f WHERE f.Fcode='G' AND f.DepartClass='"
						+ s.get(i).get("ClassNo") + "'"));
				minu2 = Integer.parseInt(df.sqlGetStr("SELECT Money FROM FeePay f WHERE f.Fcode='F' AND f.DepartClass='"
						+ s.get(i).get("ClassNo") + "'"));
				sum = (minu1 + minu2) / 2;
			} catch (Exception e) {
				sum = 0;
	...
	public String free() {
		...
		for (int i = 0; i < s.size(); i++) {
			...
			df.exSql("DELETE FROM FeePay WHERE DepartClass='" + s.get(i).get("ClassNo") + "'AND Fcode='E'AND Kind='1'");
			df.exSql("INSERT INTO FeePay(DepartClass,Fcode,Kind,Money)VALUES('" + s.get(i).get("ClassNo")
					+ "','E','1', '-" + sum + "');");
		}
		return search();
